...
class Model2Trainer:
    ...
    def load_images_from_folder(self, folder_path):
        ...
        for category in categories:
            ...
            if category not in self.reverse_label_map:
                continue
    ...
    def train(self, X_train, y_train):
        ...
        print("\n" + "=" * 80)
        print("TRAINING MODEL 2 - MASK COLOUR CLASSIFIER")
        print("=" * 80)
    ...
    def plot_loss_curve(self, losses):
        ...
        plt.title('Model 2 - Training Loss Over Epochs', fontsize=14, fontweight='bold')
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Loss', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        output_path = 'results/model2_loss_curve.png'
    ...
    def save_metrics(self, accuracy, precision, recall, f1, cm):
        ...
        metrics_file = 'results/model2_metrics.txt'
        
        with open(metrics_file, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 2 - MASK COLOUR CLASSIFIER (FROM SCRATCH)\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("  Output Layer:   6 neurons (Softmax)\n")
    ...
    def plot_confusion_matrix(self, cm):
        ...
        plt.title('Model 2 - Mask Colour Classifier\nConfusion Matrix', fontsize=14, fontweight='bold')
        plt.ylabel('True Label', fontsize=12)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.tight_layout()
        
        output_path = 'results/confusion_matrices/model2_confusion_matrix.png'
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        print(f"✓ Confusion matrix saved to: {output_path}")
        plt.close()
    
    def save_model(self, output_path='models/model2_mask_colour.pkl'):
...
def main():
    ...
    print("\n" + "=" * 80)
    print("SCRIPT 05: TRAIN MODEL 2 - MASK COLOUR CLASSIFIER")
    print("=" * 80)
    ...
    try:
        trainer = Model2Trainer()
        
        print("\n[STEP 1] Loading Training Data")
        print("-" * 80)
        X_train, y_train = trainer.load_images_from_folder('dataset/model2_mask_colour/train')
        ...
        print("-" * 80)
        X_test, y_test = trainer.load_images_from_folder('dataset/model2_mask_colour/test')
        ...
        print(f"  Recall:    {recall:.4f}")
        print(f"  F1-Score:  {f1:.4f}")
        print(f"\n💾 Model saved to: models/model2_mask_colour.pkl")
        print(f"📊 Metrics saved to: results/model2_metrics.txt")
        print(f"📈 Loss curve saved to: results/model2_loss_curve.png")
        print("=" * 80)
        print("\n")
